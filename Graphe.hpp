...
bool Graphe<Objet>::estFortementConnexe() const {
    ...
    std::list<Sommet*> visited; // Correct type pour le DFS.
    int sommetDepart = m_listSommets->m_numero;

    _parcoursDFS(_getSommet(sommetDepart), visited);

    if (visited.size() != m_nbSommets) {
        return false;
    }

    // Vérification avec le graphe transposé
    std::list<Sommet*> visitedTranspose;
    _initEtatSommets(false);
    std::stack<int> pile;
    ...
    while (!pile.empty()) {
        ...
        if (std::find(visitedTranspose.begin(), visitedTranspose.end(), _getSommet(sommet)) != visitedTranspose.end()) {
            continue;
        }

        visitedTranspose.push_back(_getSommet(sommet));
    ...
    return visitedTranspose.size() == m_nbSommets;
}
 ...
 */
template<typename Objet>
int Graphe<Objet>::dijkstra(const Objet& p_origine, const Objet& p_destination, std::vector<Objet>& p_chemin) {


    try {
        // Étape 1 : Initialiser les sommets pour Dijkstra
        Sommet* sommetOrigine = _getSommet(getNumeroSommet(p_origine));
        Sommet* sommetDestination = _getSommet(getNumeroSommet(p_destination));
        _initPathFinding(p_origine, p_destination, sommetOrigine, sommetDestination);

        // Étape 2 : Liste des sommets à explorer (sommetsAExplorer)
        std::vector<int> sommetsAExplorer = listerSommets();

        // Étape 3 : Boucle principale
        while (!sommetsAExplorer.empty()) {
            // Trouver le sommet avec le coût minimum dans sommetsAExplorer
            Sommet* u = nullptr;
            int minCout = std::numeric_limits<int>::max();
            for (int sommetID : sommetsAExplorer) {
                Sommet* sommet = _getSommet(sommetID);
                if (sommet->m_cout < minCout) {
                    minCout = sommet->m_cout;
                    u = sommet;
                }
            }

            PRECONDITION(u != nullptr); // Le sommet avec le coût minimal doit être trouvé
            // Retirer u de sommetsAExplorer
            sommetsAExplorer.erase(std::remove(sommetsAExplorer.begin(), sommetsAExplorer.end(), u->m_numero), sommetsAExplorer.end());

            // Parcourir les voisins de u
            std::vector<int> adjacents = listerSommetsAdjacents(u->m_numero);
            for (int voisin : adjacents) {
                int tempCout = u->m_cout + getCoutArc(u->m_numero, voisin);

                // Si un chemin plus court est trouvé
                Sommet* sommetVoisin = _getSommet(voisin);
                if (tempCout < sommetVoisin->m_cout) {
                    sommetVoisin->m_cout = tempCout;
                    sommetVoisin->m_predecesseur = u;
                }
            }
        }

        // Étape 4 : Récupérer le chemin le plus court
        Sommet* courant = sommetDestination;
        if (courant->m_predecesseur != nullptr || courant == sommetOrigine) {
            // Construire le chemin en remontant les prédécesseurs
            while (courant != nullptr) {
                p_chemin.insert(p_chemin.begin(), courant->m_etiquette);
                courant = courant->m_predecesseur;
            }
        } else {
            // Aucun chemin trouvé
        ...
        // Retourner le coût total du chemin
 ...
 * \pre Les sommets du graphe doivent être numérotés et étiquetés.
 * \post Le vecteur passé en paramètre contiendra les étiquettes des sommets d'articulation.
...
void Graphe<Objet>::getPointsArticulation(std::vector<Objet>& p_sommets) {
    try {
        ...
        // Parcourir chaque sommet pour vérifier s'il est un point d'articulation
        for (unsigned int i = 0; i < sommets.size(); ++i) {
            // Créer une copie du graphe
            Graphe<Objet> grapheCopie(*this);

            // Récupérer l'étiquette du sommet actuel
            Objet etiquette = grapheCopie._getSommet(sommets[i])->m_etiquette;

            // Supprimer le sommet actuel dans la copie
            grapheCopie.enleverSommet(sommets[i]);

            // Vérifier si le graphe est toujours fortement connexe
            if (!grapheCopie.estFortementConnexe()) {
                // Ajouter l'étiquette du sommet au vecteur des points d'articulation
                p_sommets.push_back(etiquette);
            }
        }
    }
    catch (const std::bad_alloc&) {
        // Gérer une erreur de mémoire insuffisante
